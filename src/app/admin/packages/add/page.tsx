...
const BASE_PACKAGE_OPTIONS: PackageOption[] = [
  ...
  { id: "12h", label: "12 Hours", description: "Full day ride (12 hours).", isCustom: false },
  { id: "1d", label: "1 Day", description: "Unlimited day ride.", isCustom: false },
];
...
export default function AdminPackagesAddPage() {
  ...
  // Add form state
  const [customPackages, setCustomPackages] = useState<CustomPackageMeta[]>([]);
  const packageOptions = useMemo<PackageOption[]>(() => {
    const dynamic = customPackages
      .filter((item) => item && item.id)
      .map<PackageOption>((item) => ({
        id: item.id,
  ...
  const allPackageIds = useMemo(() => packageOptions.map((option) => option.id), [packageOptions]);

  const [pkg, setPkg] = useState<string>(BASE_PACKAGE_OPTIONS[0]?.id ?? "");
  const [lines, setLines] = useState<string>("");
  const [adding, setAdding] = useState(false);
  const [addMsg, setAddMsg] = useState<string | null>(null);

  // Accounts list state
  const [filterPkg, setFilterPkg] = useState<"all" | string>("all");
  ...
  useEffect(() => {
    const loadCustomPackages = async () => {
      try {
        ...
        const list: CustomPackageMeta[] = Array.isArray(data?.customPackages)
          ? data.customPackages
              .filter((item: any) => item && typeof item.id === "string" && item.id.trim().length > 0)
              .map((item: any) => ({
                id: String(item.id).trim(),
                name: typeof item.name === "string" ? item.name : "",
                blockMinutes: Number(item.blockMinutes ?? 0),
  ...
  useEffect(() => {
    if (!pkg && packageOptions.length > 0) {
      setPkg(packageOptions[0].id);
      return;
    }
    if (pkg && packageOptions.length > 0 && !packageOptions.some((option) => option.id === pkg)) {
      setPkg(packageOptions[0].id);
    }
  }, [pkg, packageOptions]);
  ...
  const pkgLabel = useCallback(
    (id: string) => {
      const found = packageOptions.find((option) => option.id === id);
  ...
  }, [filterPkg, filterStatus, token, page, fetchCounts]);
  ...
  const onAdd = async (e: React.FormEvent) => {
    ...
    setAddMsg(null);
    if (!pkg) {
      setAddMsg("Please select a package before adding accounts.");
      return;
    }
    const items = lines
      .split(/\r?\n/)
  ...
  return (
    <main className="space-y-6 p-6">
      <section className="rounded-xl bg-white p-5 shadow-sm">
        <h1 className="text-xl font-semibold text-sky-600">Add Package Accounts</h1>
        <p className="mt-1 text-sm text-slate-600">
          Add credentials for any package below (including custom tiers). Each line is one account: email,password
        </p>

        <div className="mt-4 grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {packageOptions.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
              No packages configured. Create a custom package from the Admin Settings page first.
            </div>
          ) : (
        ...
        <form onSubmit={onAdd} className="mt-5 grid gap-4 sm:grid-cols-3">
          ...
          <div className="sm:col-span-3 flex items-center gap-2">
            <Button
              type="submit"
              loading={adding}
              disabled={adding || !pkg || packageOptions.length === 0}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <div className="min-w-[160px]">
                <Select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as "all" | "active" | "sold")}>
